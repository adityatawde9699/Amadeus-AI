...
class ArgumentExtractor:
    ...
    async def _generate_code_with_llm(self, user_input: str) -> dict[str, Any]:
        """Use the LLM to generate Python code from a natural-language request."""
        prompt = (
            "You are an expert Python developer. "
            "The user wants to execute some logic using a Python script.\n\n"
            f"User request: {user_input}\n\n"
            "Generate the Python code to fulfill this request. "
            "Return ONLY the raw Python code. No markdown fences, no explanations. "
            "Ensure the code is safe and self-contained."
        )
        try:
            if self._llm_router:
                raw_code, provider = await self._llm_router.generate(
                    prompt=prompt, complexity="normal"
                )
                logger.debug("Code generated by %s", provider)
                return {"code": self._strip_fences(raw_code)}
        except Exception as exc:
            logger.warning("Code generation failed: %s", exc)
        return {"code": f"# Error generating code for: {user_input}"}
    ...
    def _strip_fences(text: str) -> str:
        # 1. Remove <think>...</think> blocks first (Qwen3 / reasoning models)
