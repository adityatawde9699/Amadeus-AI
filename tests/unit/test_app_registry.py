...
def test_app_registry_initialization_no_cache(mock_cache_dir):
    with patch.object(AppRegistry, 'scan_and_cache', return_value={"chrome": "/path/to/chrome"}) as mock_scan:
...
def test_app_registry_get_executable_substring_match():
    with patch.object(AppRegistry, '_load_cache', return_value={"visual studio code": "/path/to/code.exe"}):
        registry = AppRegistry()
