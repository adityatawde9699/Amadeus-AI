...
@pytest.fixture
def amadeus_service():
    with patch("src.app.services.amadeus_service.genai.GenerativeModel"), \
         patch("src.app.services.amadeus_service.AgentOrchestrator"): # mock agent orchestrator
        service = AmadeusService(settings=get_settings())
        return service

def test_classifier_loading_success(amadeus_service):
    with patch("src.app.services.amadeus_service.os.path.exists", return_value=True), \
         patch("src.app.services.amadeus_service.joblib.load", side_effect=["vectorizer", "classifier"]):
