...
# Suppress Streamlit's "missing ScriptRunContext" warning in async threads
class StreamlitLogFilter(logging.Filter):
    def filter(self, record):
        return "missing ScriptRunContext" not in record.getMessage()

logging.getLogger("streamlit").addFilter(StreamlitLogFilter())
logging.getLogger("streamlit.runtime.scriptrunner_utils.script_run_context").addFilter(StreamlitLogFilter())
...
from reminder_utils import list_reminders
...
def get_amadeus():
    try:
        ...
        logger.error(f"Amadeus Init failed: {e}", exc_info=True)
        return None
